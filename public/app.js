const FALLBACK_THUMBS = [
  ...
  "https://images.unsplash.com/photo-1508098682722-e99c643e7f0b?auto=format&fit=crop&w=600&q=80",
    ...
    <article class="story">
      ...
      <div class="story-body">
        ...
        <div class="story-meta">
          <span class="story-badge">${esc(sourceBadge(item.sourceType))}</span>
          ${esc(item.source)} · ${typeof item.ageHours === "number" ? `${item.ageHours}h ago` : "recent"}
        </div>
      </div>
    </article>
  `;
...
async function loadNews() {
  ...
  try {
    ...
    if (!Array.isArray(news) || news.length === 0) {
      hero.innerHTML = `
        <h1 class="hero-title">No fresh Serbian football news right now</h1>
        <div class="hero-meta">Try again soon</div>
      `;
      aiNews.innerHTML = `<div class="empty-state">No stories available.</div>`;
      topNews.innerHTML = `<div class="empty-state">No stories available.</div>`;
      return;
    }
    ...
    topNews.innerHTML = news.slice(5, 11).map((item, i) => storyCard(item, i + 5)).join("");
  } catch (err) {
    console.error(err);
    hero.innerHTML = `
      <h1 class="hero-title">Error loading latest Serbian football news</h1>
      <div class="hero-meta">Please refresh</div>
    `;
    aiNews.innerHTML = `<div class="empty-state">Feed error.</div>`;
    topNews.innerHTML = `<div class="empty-state">Feed error.</div>`;
  }
}
